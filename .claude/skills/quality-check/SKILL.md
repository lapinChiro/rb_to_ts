---
name: quality-check
description: 作業完了時 (コミット前) の品質チェック手順。lint, type-check, knip, test を全て実行し 0 エラー・0 警告を確認する
user-invocable: true
---
...
npx knip > /tmp/knip-result.txt 2>&1
npx vitest run > /tmp/test-result.txt 2>&1
```
...
- lint 警告を `eslint-disable` で黙らせること (根本原因を修正する)
- 品質チェックを実行せずに「完了」と報告すること
- 出力を流し読みして「問題なさそう」と判断すること。各コマンドの終了メッセージまで確認する
- 「今回の変更に起因しない」「スコープ外」を理由に、発見した警告・エラーの修正を先送りすること

## 検証

- 4 コマンド全てが終了コード 0 で完了している
